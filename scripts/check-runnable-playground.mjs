...
const supported = new Set([...languagesSource.matchAll(/language\("([a-z+#]+)"/gu)].map(([, language]) => language));
const documentedFences = [...playground.matchAll(/^```run-([a-z+#]+)\s*$/gmu)].map(([, language]) => language);
