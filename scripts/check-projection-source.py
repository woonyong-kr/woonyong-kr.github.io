...
expected = {artifact.relative_path.as_posix(): artifact.content for artifact in report.artifacts}
...
print(f'Producer bytes: {len(report.documents)} documents, {len(report.redirects)} redirects; missing={len(missing)}, extra={len(extra)}, changed={len(changed)}')
